    ...
    <div class="modal-dialog" style="width: 97%">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-info" id="button_switch" onclick="switchDisplay()">合并</button>
...
<div>
    ...
    <div style="height: 20px;width: 100%"></div>
    <div class="row">
        <div class="col-3" style="padding: 0 5px 0 5px;line-height: 20px;font-size: 18px">应收：</div>
        <div class="col-2" id="income_info" style="padding: 0 5px 0 5px;line-height: 20px;font-size: 18px"></div>
        <div class="col-2" style="padding: 0 5px 0 5px"></div>
        <div class="col-3" style="padding: 0 5px 0 5px;line-height: 20px;font-size: 18px">实收：</div>
        <div class="col-2" id="income_info_real" style="padding: 0 5px 0 5px;line-height: 20px;font-size: 18px"></div>
    </div>
    <div style="height: 20px;width: 100%"></div>

    <div id="div_table" style="height: calc(100vh - 240px);width: 100%;overflow-y: auto">
...
<script type="text/html" id="finish-template">
    ...
    <div class="row" data-bid="{%= list.finish_info[i].Bill_ID %}" onclick="deleteBill(this)">
    ...
    <div class="row" style="padding-top: 7px;">
        <div class="col-4" style="text-align: left;padding-left: 0;padding-right: 10px;font-weight: bold">总金额</div>
        <div class="col-5"></div>
...
<script type="text/html" id="finish-view-template">
    ...
    <div class="row" style="font-weight: bold">
        <div class="col-4" style="text-align: left;padding-left: 0;padding-right: 10px">实收金额</div>
        <div class="col-5"></div>
        {% if(list.finish_info[0].Bill_Equal_Sale_Real == null){ %}
        <div class="col-3" id="real-finish" style="text-align: center;padding-left: 0;padding-right: 10px" data-id="{%= list.finish_info[0].Bill_Equal_ID %}">{%= list.finish_info[0].Bill_Equal_Sale %}</div>
        {% }else { %}
...
<script>
    ...
    getBillEqualTable()
    setTimeout(getIncomeInfo(), 10000);
    ...
    function getIncomeInfo() {
        let _formData = new FormData;
        _formData.append('_token', "{{ csrf_token() }}");
        _formData.append('bill_date', dateInfo.Date);

        fetch("/epos/get_income_info", {method: 'post', body: _formData}).then(function (_res) {
            return _res.json();
        }).then(function (_resJson) {
            console.log(_resJson);
            if (_resJson.status == "success"){
                document.getElementById("income_info").innerHTML = _resJson.data.income_info[0].Bill_Equal_Sale
                document.getElementById("income_info_real").innerHTML = _resJson.data.income_info[0].Bill_Equal_Sale_Real
            }else {
                layer.msg("出错了！错误原因：" + _resJson.message);
            }
        })
    ...
    function switchDisplay() {
        let Bill_Equal_ID = document.getElementById("get_id").getAttribute("data-eid");
        let _formData = new FormData;
        _formData.append('_token', "{{ csrf_token() }}");
        _formData.append('Bill_Equal_ID',Bill_Equal_ID);
        fetch("/epos/get_finish_view", {method: 'post', body: _formData}).then(function (_res) {
            ...
            if (_resJson.status == "success"){
                ...
                document.getElementById("button_switch").setAttribute("onclick","switchEdit()");
                document.getElementById("button_switch").innerHTML = "展开";
                document.getElementById("ydButton").setAttribute("onclick","switchLog()");
                document.getElementById("ydButton").className = "btn btn-secondary";
            }else {
                layer.msg("出错了！错误原因：" + _resJson.message);
            }
        })
    ...
    function switchEdit() {
        ...
        fetch("/epos/get_finish", {method: 'post', body: _formData}).then(function (_res) {
            ...
            if (_resJson.status == "success"){
                ...
                document.getElementById("button_switch").setAttribute("onclick","switchDisplay()");
                document.getElementById("button_switch").innerHTML = "合并";
                document.getElementById("ydButton").setAttribute("onclick","confirmFinish()");
                document.getElementById("ydButton").className = "btn btn-danger";
            }else {
                layer.msg("出错了！错误原因：" + _resJson.message);
            }
        })
    ...
    function switchLog() {
        layer.msg("合并模式下无法结单！切换回普通模式后再结单吧", {
            zIndex:10000,
        })
    }
    ...
    function deleteBill(bill) {
        // console.log(bill.getAttribute('data-bid'))
        // console.log(bill.firstElementChild.innerHTML)
        let bill_id = bill.getAttribute('data-bid');
        let bill_name = bill.firstElementChild.innerHTML;
        layer.confirm('确认删除' + bill_name + '？', {
            btn: ['取消','确认'] //按钮
        }, function(){
            layer.msg('已取消');
        }, function(){
            ...
            let _formData = new FormData;
            _formData.append('_token', "{{ csrf_token() }}");
            _formData.append('Bill_ID',bill_id);
            fetch("/epos/delete_bill_dish", {method: 'post', body: _formData}).then(function (_res) {
                return _res.json();
            }).then(function (_resJson) {
                console.log(_resJson);
                if (_resJson.status == "success"){
